...
```
~/.millwright/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.millwright/sessions/`.

Millwright also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Millwright uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Millwright AI)
...
### Extended Message Types (from Millwright Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Millwright (legacy field name)
