...
<h1 align="center">Millwright CLI</h1>

<p align="center">
  RLM-native terminal coding and research harness.
</p>

Millwright began as a hard fork of [pi-mono](https://github.com/badlogic/pi-mono), but it is now developed and distributed independently. This workspace retains inherited `@earendil-works/pi-*` source package identifiers, the `pi` package manifest key, and a source-package `pi` bin entry for internal compatibility. Public releases are versioned npm artifacts installed as `millwright-agent`; release packaging rewrites the application package and command to `millwright`. Do not use the inherited workspace package as the Millwright install path.
...
- [Customization](#customization)
  ...
  - [Themes](#themes)
  - [Millwright Packages](#millwright-packages)
...
Then just talk to Millwright. By default, Millwright gives the model one tool: `ipython`. The model uses the persistent kernel to read files, run commands, edit code, and inspect data. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [Millwright packages](#millwright-packages).

The Python kernel runtime is set up automatically on first invocation. Set `MILLWRIGHT_KERNEL_PYTHON` to use an existing Python environment with `ipykernel`.

**Platform notes:** [Windows](docs/windows.md) | [Termux (Android)](docs/termux.md) | [tmux](docs/tmux.md) | [Terminal setup](docs/terminal-setup.md) | [Shell aliases](docs/shell-aliases.md)

## Providers & Models

For each built-in provider, Millwright maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
**Custom providers & models:** Add providers via `~/.millwright/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.millwright/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Millwright can receive the follow-up shortcut.
...
Sessions auto-save as flat JSONL files under `~/.millwright/sessions/`. Each session header records its working directory, which the searchable session view uses to identify and open saved sessions.

```bash
millwright -c                  # Continue most recent session
millwright -r [path|id]        # Browse past sessions or resume one directly
millwright --no-session        # Ephemeral mode (don't save)
millwright --fork <path|id>    # Fork specific session file or ID into a new session
```
...
| Location | Scope |
|----------|-------|
| `~/.millwright/settings.json` | Global (all projects) |
| `.millwright/settings.json` | Project (overrides global) |

See [docs/settings.md](docs/settings.md) for all options.

### Update checks

Millwright stable builds fetch `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` and remain on the beta channel. Override the base URL with `MILLWRIGHT_DOWNLOAD_BASE_URL`. Disable version checks with `MILLWRIGHT_SKIP_VERSION_CHECK=1`.

Use `--offline` or `MILLWRIGHT_OFFLINE=1` to disable startup network operations, including update checks and package update checks.

## Context Files

Millwright loads `AGENTS.md` (or `CLAUDE.md`) at startup from:
- `~/.millwright/AGENTS.md` (global)
...
Replace the default system prompt with `.millwright/SYSTEM.md` (project) or `~/.millwright/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.millwright/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.millwright/prompts/`, `.millwright/prompts/`, or a [Millwright package](#millwright-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).

### Skills

On-demand capability packages following the [Agent Skills standard](https://agentskills.io). At startup, Millwright gives the model each visible skill's name, type, description, and location. The full `SKILL.md` stays out of context until the model inspects it with `ipython` or you explicitly invoke `/skill:name`.

```markdown
<!-- ~/.millwright/skills/my-skill/SKILL.md -->
---
name: my-skill
...
Skills can also be Python-backed. A Python skill is a normal skill directory with `SKILL.md` plus a Python package at `src/<import_name>/`. Millwright installs it into the persistent IPython kernel and exposes it by import name, so the model can call it directly, inspect it with `help()`, or use any console scripts the skill declares.

Place in `~/.millwright/skills/`, `~/.agents/skills/`, `.millwright/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [Millwright package](#millwright-packages) to share with others. See [docs/skills.md](docs/skills.md).

Millwright ships with a built-in `websearch` skill (Google search via the [Serper](https://serper.dev) API). It loads by default; run `/login`, switch to **MCP Connections**, and choose "Serper (web search)" to add your key. Disable it with `bundledSkills.websearch: false`, or override it with your own `websearch` skill in any location above. See [docs/skills.md#built-in-skills](docs/skills.md#built-in-skills).
...
Credentials are stored once in `~/.millwright/auth.json` (under `mcp:<name>`); the kernel reads them directly and the host refreshes expired tokens. Enablement is derived from whether valid credentials exist, so there is no separate on/off switch.
...
```python
# ~/.millwright/skills/acme/src/acme/__init__.py
...
TypeScript modules that extend Millwright with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. Millwright waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make Millwright look like Claude Code
- Games while waiting (yes, Doom runs)
- ...anything you can dream up

Place in `~/.millwright/extensions/`, `.millwright/extensions/`, or a [Millwright package](#millwright-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and Millwright immediately applies changes.

Place in `~/.millwright/themes/`, `.millwright/themes/`, or a [Millwright package](#millwright-packages) to share with others. See [docs/themes.md](docs/themes.md).
...
Without a `pi` manifest, Millwright auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "millwright-agent";
...
Millwright is forked from [pi-mono](https://github.com/badlogic/pi-mono) by Mario Zechner and keeps MIT attribution in the root license.

The package architecture, extension model, and source package names still reflect that upstream lineage while the distributed command and release artifacts are branded for Millwright.
...
Run `millwright help` for the command list and `millwright help <command>` for details.

### Agent Commands

```bash
millwright agents                         # Search running, idle, and inactive sessions
millwright list [--all]                   # List active or saved agents
millwright attach <agent>                 # Attach the interactive UI
millwright stop <agent>                   # Stop one agent
millwright rename <agent> <name>          # Rename an agent
millwright send <agent> <message>         # Send an agent-to-agent message
millwright status                         # Show background service status
millwright doctor [--fix]                 # Inspect or safely clean up background services
millwright shutdown [--force]             # Stop every agent, worker, and background service
```
...
```bash
millwright schedule list [--all] [agent]
millwright schedule add <agent> <schedule> -- <message>
millwright schedule cancel <job-id>
```
...
millwright update [--force]                   # Update Millwright itself
millwright config                             # Enable/disable package resources
```
...
Use `millwright session export <file> [output]` to export a saved session to HTML.
...
# High thinking level
millwright --thinking high "Solve this complex problem"
```
...
|----------|-------------|
| `MILLWRIGHT_CODING_AGENT_DIR` | Override config directory (default: `~/.millwright`) |
...
| `MILLWRIGHT_PACKAGE_DIR` | Override package directory (useful for Nix/Guix where store paths tokenize poorly) |
...
| `MILLWRIGHT_TELEMETRY` | Override pseudonymous aggregate usage analytics with `1`/`true`/`yes` or `0`/`false`/`no` |
| `MILLWRIGHT_TELEMETRY_ENDPOINT` | Override the aggregate analytics ingestion endpoint |
...
| `MILLWRIGHT_CACHE_RETENTION` | Set to `long` for extended prompt cache (Anthropic: 1h, OpenAI: 24h) |
...
| `MILLWRIGHT_KERNEL_PYTHON` | Use an existing Python environment with `ipykernel` instead of auto-bootstrapping `~/.millwright/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

Inherited compatibility variables are still read by the current runtime. They do not change the application name, command, or default `~/.millwright` configuration path.
...
- [Millwright AI](../ai): Core LLM toolkit
- [Millwright Core](../agent): Agent framework
- [Millwright TUI](../tui): Terminal UI components
