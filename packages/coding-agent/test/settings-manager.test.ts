...
describe("SettingsManager", () => {
	...
	describe("autoRefine", () => {
		it("defaults to disabled while preserving explicit opt-in and opt-out", () => {
			const manager = SettingsManager.create(projectDir, agentDir);
			...
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ autoRefine: { enabled: true } }));
			const optedIn = SettingsManager.create(projectDir, agentDir);

			expect(optedIn.getAutoRefineSettings().enabled).toBe(true);
	...
	describe("error tracking", () => {
		...
		it("should report a new project error when saving after the load error was drained", async () => {
			const settingsPath = join(projectDir, ".millwright", "settings.json");
		...
		it("drains only the requested scope", () => {
			...
			writeFileSync(join(projectDir, ".millwright", "settings.json"), "{ invalid project json");
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("project settings directory creation", () => {
		it("should not create .millwright folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .millwright folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .millwright folder that beforeEach created
			rmSync(join(projectDir, ".millwright"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .millwright folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".millwright"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .millwright folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .millwright folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .millwright folder should NOT exist yet
			expect(existsSync(join(projectDir, ".millwright"))).toBe(false);
			...
			// Now .millwright folder should exist
			expect(existsSync(join(projectDir, ".millwright"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".millwright", "settings.json"))).toBe(true);
		});
	});
	...
	describe("idle worker eviction", () => {
		...
		it("reads and writes the global daemon policy without project overrides", async () => {
			...
			writeFileSync(join(projectDir, ".millwright", "settings.json"), JSON.stringify({ idleEvictionMinutes: 30 }));
			const manager = SettingsManager.create(projectDir, agentDir);
