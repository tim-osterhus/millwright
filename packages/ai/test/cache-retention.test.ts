...
describe("Cache Retention (MILLWRIGHT_CACHE_RETENTION)", () => {
	const originalEnv = process.env.MILLWRIGHT_CACHE_RETENTION;
	...
	afterEach(() => {
		if (originalEnv !== undefined) {
			process.env.MILLWRIGHT_CACHE_RETENTION = originalEnv;
		} else {
			delete process.env.MILLWRIGHT_CACHE_RETENTION;
		}
	});
	...
	describe("Anthropic Provider", () => {
		it.skipIf(!process.env.ANTHROPIC_API_KEY)(
			"should use default cache TTL (no ttl field) when MILLWRIGHT_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.ANTHROPIC_API_KEY)(
			"should use 1h cache TTL when MILLWRIGHT_CACHE_RETENTION=long",
			async () => {
	...
	describe("OpenAI Responses Provider", () => {
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should not set prompt_cache_retention when MILLWRIGHT_CACHE_RETENTION is not set",
			async () => {
		...
		it.skipIf(!process.env.OPENAI_API_KEY)(
			"should set prompt_cache_retention to 24h when MILLWRIGHT_CACHE_RETENTION=long",
			async () => {
				process.env.MILLWRIGHT_CACHE_RETENTION = "long";
				const model = getModel("openai", "gpt-4o-mini");
				let capturedPayload: any = null;
