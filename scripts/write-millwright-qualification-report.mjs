...
function assertRecordDetails(id, details) {
	...
	if (id === "version") {
		...
		if (details.product !== "Millwright" || details.version !== "0.0.3") fail("installed version identity is invalid");
		return;
	}
	if (id === "tui") {
...
function validateInstalledQualification(installed, verified, expectedSha256) {
	...
	if (installed.artifact?.filename !== "millwright-agent-0.0.3.tgz" || installed.artifact?.sha256 !== verified.sha256 || installed.artifact?.integrity !== verified.integrity) fail("installed report artifact disagrees with verifier");
